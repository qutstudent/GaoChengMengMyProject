...
<title><%=newJcpeizhi.getSjduochuBieming()%></title>
...
var url;
	function searchSjduochu(){
		$('#dg').datagrid('load',{
			sjduochuName:$('#s_sjduochuName').val()
		});
	}
	...
	function openSjduochuAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加<%=newJcpeizhi.getSjduochuBieming()%>");
		url="../addSjduochu?yonghuId=<%=yonghuId %>";
	}
	...
	function openSjduochuModifyDialog(){
		...
		$("#dlg").dialog("open").dialog("setTitle","编辑<%=newJcpeizhi.getSjduochuBieming()%>");
		$("#fm").form("load",row);
		url="../addSjduochu?sjduochuId="+row.sjduochuId+"&userId="+userId;
	}
	...
	function daoruSjduochus(){
		$("#daoru").dialog("open").dialog("setTitle","导入<%=newJcpeizhi.getSjduochuBieming()%>");
	...
	function shangchuanSjduochu(){
		...
		var row=selectedRows[0];
		$("#shangchuan").dialog("open").dialog("setTitle","上传<%=newJcpeizhi.getSjduochuBieming()%>");
	...
	function chakanNeirong(sjduochuId, row) {
		return '<a style="color:red;"target="_blank" href="<%=basePath%>wangzhan/sjduochushow.jsp?sjduochuId=' + sjduochuId + '">查看' + '</a>';  
	}
</script>
</head>
<body style="margin: 5px;">
<!--startprint-->
	<table id="dg" title="<%=newJcpeizhi.getSjduochuBieming()%>" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSjduochus?yonghuId=<%=yonghuId %>" fit="true" rownumbers="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="sjduochuId" width="10" formatter="chakanNeirong">查看</th>
				<th field="sjduochuName" width="40">名称</th>
				<th field="sjduochuMark1" width="40">电话</th>
				<th field="sjduochuDouble" width="10">工资</th>
	...
	<div id="tb">
		...
		<div>
		&nbsp;名称：&nbsp;<input type="text" name="s_sjduochuName" id="s_sjduochuName" size="10"/>
		...
		<form id="fm" method="post">
			<table cellspacing="5px;">
				...
				<tr>
					...
					<td>工资：</td>
					<td><input type="text" name="sjduochuDouble" id="sjduochuDouble" class="easyui-validatebox" required="true"/></td>
				</tr>
				<tr>
					<td valign="top"><%=newJcpeizhi.getSjduochuBieming()%>详情：</td>
